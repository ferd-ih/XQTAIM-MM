#from https://github.com/superlouis/GATGNN/blob/master/gatgnn/model.py
...
class CLUSTER_Attention(nn.Module):
    def __init__(self,neurons_1,neurons_2,num_cluster, cluster_method = 'random'):
        ...
        self.learn_unpool       = Linear(2*neurons_1+1,num_cluster)        
            ...
            if   len(graph_features) == 1: 
                ...
                if self.clustering_method == 'fixed':        #--- GI M-2
                    ...
                    region_input = torch.cat([
                        graph_features,
                        total_feat.repeat(graph_features.size(0), 1),
                        clus_t.unsqueeze(1).float()
                    ], dim=-1)
                    # random_sets  = torch.argmax(F.softmax(self.learn_unpool(region_input)),dim=1).tolist()
                    random_sets  = torch.argmax(F.softmax(self.learn_unpool(region_input), dim=1), dim=1).tolist()
                # normalized-regions
                unique, counts = np.unique(region_arr, return_counts=True) 
                counts         = counts/counts.sum()
                sets_dict      = dict(zip(unique, counts))             
                random_ratio   = torch.tensor([sets_dict[i] for i in random_sets],dtype=torch.float32).cuda()
...
class GATGNN(torch.nn.Module):
    def __init__(self,data,heads=4,classification=None,neurons=64,nl=3,xtra_layers=True,global_attention='composition',
        ...
        self.embed_n        = Linear(113,n_h)
        self.embed_e        = Linear(50,n_h)
    ...
    def forward(self,data):
        ...
        # print("WE INSIDE GATGNN")
        batch, global_feat,cluster = data.batch,data.global_feature,data.cluster
        # print(f"x dtype: {x.dtype}, cluster dtype: {cluster.dtype}, batch dtype: {batch.dtype}")
        x           = self.embed_n(x)
