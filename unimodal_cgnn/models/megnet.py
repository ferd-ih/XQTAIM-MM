...
class Megnet_NodeModel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        # row, col = edge_index
        # print("***INSIDE MEGNET NOW BEWARE***")
        v_e = scatter_mean(edge_attr, edge_index[0, :], dim=0)
        # print(f"x shape: {x.shape}, v_e shape: {v_e.shape}, u[batch] shape: {u[batch].shape}")
        # print(f"[DEBUG] x.shape = {x.shape}")  # Should be [num_nodes, feat_dim]
        # print(f"[DEBUG] v_e.shape = {v_e.shape}")  # Should also be [num_nodes, feat_dim]
        # print(f"[DEBUG] u[batch].shape = {u[batch].shape}")  # Should match x
        # print(f"[DEBUG] edge_index.shape = {edge_index.shape}")  # Check shape [2, num_edges]
        # print(f"[DEBUG] edge_attr.shape = {edge_attr.shape}")  # Should be [num_edges, feat_dim]
