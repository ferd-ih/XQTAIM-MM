...
class GATGNN_GIM1_globalATTENTION(torch.nn.Module):
    def __init__(self, dim, act, batch_norm, batch_track_stats, dropout_rate, fc_layers=2):
        ...
        for i in range(self.fc_layers + 1):
            if i == 0:
                # Use dim+103 for global_feature (gatgnn processing) or dim+108 for glob_feat (deep_gatgnn processing)
                lin    = torch.nn.Linear(dim+103, dim)
    ...
    def forward(self, x, batch, glbl_x ):
        # Handle batching: expand per-graph features to per-node (like gatgnn.py's COMPOSITION_Attention)
        # When batched, glob_feat becomes [batch_size, 108] instead of [total_nodes, 108]
        if glbl_x.shape[0] != x.shape[0]:
            counts = torch.unique(batch, return_counts=True)[-1]
            glbl_x = torch.repeat_interleave(glbl_x, counts, dim=0)
...
class DEEP_GATGNN(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Handle scalar targets (0-d tensor after GetY transform) and multi-target cases
        if data[0].y.ndim == 0:
            output_dim = 1
        elif data[0].y.ndim == 1:
            # 1D tensor: use the shape directly
            output_dim = data[0].y.shape[0] if data[0].y.shape[0] > 0 else 1
        else:
    ...
    def forward(self, data):
        ...
        # print(out_x.shape)
        # exit()
        out_a       = self.global_att_LAYER(out_x,data.batch,data.global_feature)
        out_x       = (out_x)*out_a            
